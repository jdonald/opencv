...
macro(ocv_check_compiler_optimization OPT)
  message("calling ocv_check_compiler_optimization(${OPT})")
  if(NOT DEFINED CPU_${OPT}_SUPPORTED)
    message("NOT DEFINED CPU_${OPT}_SUPPORTED")
    if((DEFINED CPU_${OPT}_FLAGS_ON AND NOT "x${CPU_${OPT}_FLAGS_ON}" STREQUAL "x") OR CPU_${OPT}_TEST_FILE)
      ...
      if(CPU_${OPT}_TEST_FILE)
        ...
        if(CPU_BASELINE_DETECT)
          ...
          message("just ran ocv_check_compiler_flag(CXX ${CPU_BASELINE_FLAGS} with ${CPU_${OPT}_TEST_FILE} and result was ${_varname}")
          if(${_varname})
            message("appending")
...
foreach(OPT ${CPU_KNOWN_OPTIMIZATIONS})
  ...
  foreach(OPT2 ${CPU_BASELINE_DISABLE})
    ...
    message("in foreach, __is_disabled = ${__is_disabled}")
    if(__is_disabled)
      break()
    endif()
  endforeach()
  if(__is_disabled)
    ...
    message("conditionally adding based on supported and detect: ${CPU_${OPT}_SUPPORTED} and ${CPU_BASELINE_DETECT}")
    if(CPU_${OPT}_SUPPORTED AND CPU_BASELINE_DETECT)
      message("appending to cpu baseline final: ${OPT}")
      list(APPEND CPU_BASELINE_FINAL ${OPT})
    endif()
    ocv_is_optimization_in_list(__is_from_baseline ${OPT} ${CPU_BASELINE_REQUIRE})
    if(NOT __is_from_baseline)
      message("now chec:ing baseline: ${CPU_BASELINE}")
  ...
  message("three conditions: __is_from_dispatch=${__is_from_dispatch} __is_from_baseline=${__is_from_baseline} and CPU_BASELINE_DETECT=${CPU_BASELINE_DETECT}")
  if(__is_from_dispatch OR __is_from_baseline OR CPU_BASELINE_DETECT)
    message("calling ocv_check_compiler_optimization on ${OPT}")
